...
const deleteMessage = computed(() =>
    selectedFile.value ? `Delete backup file ${selectedFile.value.name}?` : "Delete selected backup file?"
);
const restoreMessage = computed(() =>
    selectedFile.value
        ? `Restoring ${selectedFile.value.name} will overwrite current database data. Continue?`
        : "Restoring will overwrite current database data. Continue?"
);
</script>
...
<template>
    <AdminLayout title="Backup & Restore" :user-name="userName">
        <template #header>
            <PageHeader title="Backup & Restore" subtitle="Create, download, restore, and delete system backup snapshots." icon="backup" />
        </template>
